...
def build_extended_qubo(target_structure, stems, c_coeffs):
    ...
    # 5. Thermodynamic Loop Penalty (Phase 22 Surrogate HUBO)
    try:
        import numpy as np
        loop_coeffs = np.load("loop_penalty_coeffs.npy")
    except:
        print("WARNING: loop_penalty_coeffs.npy not found!")
        loop_coeffs = None
        
    def apply_loop_stack_qubo(p1, p2, p3, p4, coeffs):
        q_vars = []
        for pos in [p1, p2, p3, p4]:
            for b in ["A", "C", "G", "U"]:
                q_vars.append(f"x_{pos}_{b}")
        ...
        for a in range(16):
            var = q_vars[a]
            Q_ext[(var, var)] += coeffs[c_idx]
            c_idx += 1
            
        # 96 Quadratic terms
        for p_idx_A in range(4):
            for p_idx_B in range(p_idx_A + 1, 4):
                vars_A = q_vars[p_idx_A * 4 : (p_idx_A + 1) * 4]
                vars_B = q_vars[p_idx_B * 4 : (p_idx_B + 1) * 4]
                for vA in vars_A:
                    for vB in vars_B:
                        key = tuple(sorted([vA, vB]))
                        Q_ext[key] += coeffs[c_idx]
                        c_idx += 1
                        
        return local_offset

    if loop_coeffs is not None:
        ...
        for i in range(len(loop_indices)):
            for j in range(i+1, len(loop_indices)):
                ...
                # Check if this forms a candidate internal stack (4 bases)
                if (idx1 + 1 in loop_indices_set) and (idx2 - 1 in loop_indices_set):
                    # They must be distinct bases, and distance > 3 for hairpin bend
                    if (idx2 - 1) > (idx1 + 1):
                        offset += apply_loop_stack_qubo(idx1, idx1+1, idx2-1, idx2, loop_coeffs)
