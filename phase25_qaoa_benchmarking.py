...
from phase15_full_evaluation import fill_loops

def build_qp_from_dict(Q_dict, name="RNA_QUBO"):
...
def run_sample_benchmark():
    ...
    for target_structure in targets:
        ...
        print("\n--- 1. Constructing QUBO ---")
        # Reduced penalty_weight from 100 to 10 for QAOA
        # (SA handles penalty=100 fine, but it drowns out the physics signal for QAOA)
        Q_norm, offset_norm = build_approx_qubo(stems, c_coeffs, penalty_weight=10)
        qp_norm = build_qp_from_dict(Q_norm, name="Simple_QUBO")
        print(f"Qubits required: {qp_norm.get_num_vars()}")
        
        if qp_norm.get_num_vars() > 25:
            print(f"WARNING: {qp_norm.get_num_vars()} Qubits is too large for local Aer Simulator. Skipping to next structure...")
            continue

        print("\n--- 2. Running QAOA ---")
        # reps=1 (only 2 variational parameters — much easier for COBYLA to optimize)
        executor = SimpleQAOAExecutor(reps=1, maxiter=500, use_rqaoa=False, use_ibm_hardware=False)
        top_samples = executor.run_qaoa_with_samples(qp_norm, top_n=50)
        ...
        # Loop over all the top sequences returned by QAOA
        for rank, (x_n, e_n, prob) in enumerate(top_samples):
            var_dict = {var.name: int(val) for var, val in zip(qp_norm.variables, x_n)}
            
            pairs_list = decode_bits_to_pairs(var_dict, stems)
            if not pairs_list:
                continue
                
            hybrid_candidates = fill_loops(
                target_structure=target_structure, 
                pair_assignment=pairs_list, 
                pool_size=1000, 
                num_output=5, 
                use_penalty=True
            )
